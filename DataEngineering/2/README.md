...
2. 크게 Name node와 Data nodes들로 구성되어 있다. 
Name node - 일종의 master node로 파일에 대한 메타데이터를 저장하는 노드로 디렉토리 구조, 파일에 대한 각종 메타 데이터, 물리적 파일이 저장되어 있는 위치등을 저장한다. 
...
서로 3초 주기로 메시지를 주고받으며, Data node의 상황을 체크한다. Data node가 사용할 수 없는 상태라고 판단되면, 해당 블럭과 동일한 블럭을 가지고 있는 Data node는 다른 Data node에게 전송하여 3개의 블럭을 유지하도록 한다. 

## Hadoop의 병렬처리, Map & Reduce
1. 저장환경 위에서 병렬로 데이터를 처리하는 프레임워크<br>
